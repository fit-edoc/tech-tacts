...
const Nav = () => {
  ...
  return (
    <>
      <motion.div
        className="nav  md:h-[70px] w-[85%] fixed z-30 mx-auto mt-3 left-[7.9%] rounded-full bg-black md:bg-red-400 flex items-center"
        initial={{ width: "0%", opacity: 0 }}
        animate={{ width: "85%", opacity: 1 }}
        transition={{ duration: 0.9, ease: "easeInOut" }}
      >
        <div className="h-full w-[50%] flex justify-start">
          <h1 className="my-auto mx-2 text-lg md:text-[2.5vw]  font-head text-white">
            TechTacts
          </h1>
        </div>
        ...
        <div className="menu mr-4  block md:hidden">
          {" "}
          <motion.button onClick={() => setClicked(!clicked)}>
            {" "}
            {clicked ? (
              <RxCross2 className="text-white  text-3xl rounded-full border-2 border-white transition-all" />
            ) : (
              <motion.svg
                xmlns="http://www.w3.org/2000/svg"
                x="0px"
                className="fill-white rounded-full border-2 border-white py-1"
                y="0px"
                width="30"
                height="30"
                viewBox="0 0 30 30"
      ...
      <AnimatePresence>
        {clicked && (
          <motion.div
           ...
           transition={transtiton} // Smooth transition
          ...
          >
            <div className="mobile h-[90%] w-[90%] opacity-100 md:opacity-0  uppercase  px-2 flex flex-col items-center justify-evenly  list-none left-[5%] bg-white/5 backdrop-blur-xl absolute top-[10%]">
              <div className="w-full h-[40px] border-2 mt-2 border-white ">
                {" "}
                <HashLink to="#hero">
                  <li className="bg-white/50 text-black px-2">Home</li>
                </HashLink>
              </div>
              <div className="w-full h-[40px] border-2 border-white">
                {" "}
                <HashLink to="#offer">
